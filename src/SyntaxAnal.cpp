...
SyntaxAnal::SyntaxAnal (const char* filename) {

	wa = new WordAnal (filename);
	wa->readLine ();
...
void SyntaxAnal::analyzeElem () {
	...
	int i;
	while (t.sym == $IDENT || t.sym == $NUMBER
		|| t.sym == $LPAIR) {
...
void SyntaxAnal::generateFile (const char* filename) {
	...
	strcpy (targetSym, _Drive);
	strcat (targetSym, _Dir);
	strcat (targetSym, _Filename);
	strcat (targetSym, ".sym");

	strcpy (targetCode, _Drive);
	strcat (targetCode, _Dir);
	strcat (targetCode, _Filename);
	strcat (targetCode, ".pl0");

	FILE* fp1, * fp2;

	if ((fp1 = fopen (targetSym, "wb")) == NULL) {
	...
	if ((fp2 = fopen (targetCode, "wb")) == NULL) {
